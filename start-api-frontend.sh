...
warn_if_fabric_channel_unavailable() {
    ...
    if ! docker exec cli peer channel getinfo -c channel-org >/dev/null 2>&1; then
        echo -e "${YELLOW}⚠️  Blockchain containers are running but channel-org is unavailable${NC}"
        echo -e "${YELLOW}   Blockchain endpoints will fail until channel is recovered.${NC}"
        echo -e "${YELLOW}   Try: cd ownership-network-master && ./net.sh channel${NC}"
        echo -e "${YELLOW}   If that fails: cd ownership-network-master && ./net.sh reset && ./net.sh up${NC}"
    fi
}
