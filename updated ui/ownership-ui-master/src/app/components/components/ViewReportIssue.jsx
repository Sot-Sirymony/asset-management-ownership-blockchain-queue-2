    export default function ViewReportIssue({ onClose, record }) {
        if (!record) return null; // Return null if no record is selected
                        ...
                        <div className="relative bg-white rounded-lg shadow h-full flex flex-col">
                            ...
                            <div className="p-4 md:p-5 flex-grow custom-scroll no-scrollbar overflow-y-auto">
                                <div className="flex gap-3">
                                    <div>
                                        <span className="font-semibold text-lg leading-loose">
                                            View Detail Report Issue
                                        </span>
                                        <h3 className="mb-5 text-sm font-normal text-gray-500 dark:text-gray-400">
                                            View detailed report of the issue reported by the user
                                        </h3>
                                    </div>
                                </div>
                                <hr />
                                <div className="flex gap-3 mt-5 mb-5 rounded-md items-center">
                                    <img
                                        src={record.username.profileImg || "https://via.placeholder.com/150"}
                                        alt={record.assetName}
                                        className="w-16 h-16 rounded-full object-cover"
                                    />
                                    <div>
                                        <h1 className="text-[16px] font-semibold">{record.username.fullName || "User Name"}</h1>
                                        <p className="text-xs font-medium text-[#7f7f7f]">{record.username.email || "user@example.com"}</p>
                                    </div>
                                </div>
                                <div className="mb-3">
                                    <label
